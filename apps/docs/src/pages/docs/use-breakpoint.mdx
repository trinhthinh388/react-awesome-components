...
export const ExampleWIithCustomConfig = () => {
  const [ref, setRef] = useState(null)
...
**useBreakpoint** returns some utilities:
...
<div ref={setRef} className="w-full flex flex-col items-center justify-center">
  ...
  <div
    ...
    className="h-32 w-full max-w-32 flex items-center justify-center  p-0.5 overflow-hidden text-xl font-medium text-gray-900 rounded-lg"
  />
</div>
) }
