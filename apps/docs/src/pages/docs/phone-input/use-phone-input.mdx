...
When `country` is provided then the enterred value is formatted based on the provided selected country code and the country detection behaviour will be disabled.
...
<div>
  <div className="relative">
    <input
      ...
      placeholder="I am a fixed country phone input with international format"
      {...register('use-phone-input')}
    />
  </div>
  <h3 className="mt-3 font-bold text-xl underline underline-offset-4">
